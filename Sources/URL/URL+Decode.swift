...
extension URL.Host {
    public static func decode<T: StreamReader>(
        ...
        guard let port = try await stream.parse(Int.self) else {
            throw URL.Error.invalidPort
        }
...
extension String {
    public init<T>(removingPercentEncoding bytes: T) throws
        where T: RandomAccessCollection, T.Element == UInt8, T.Index == Int
    {
